...
public class MyTest extends LinearOpMode {
		...
		public static float deadSpace = 0.08f; //Amount of deadspace on joystick (not counted for movement)
		...
		public void runOpMode() {
				...
				boolean changedHand = false;
				boolean changedSpd = false;
				boolean changedArmPos = false;
				int adjustArmBy = -1200;
				...
				adjustArmBy = -arm.getCurrentPosition();
				arm.setPower(.23);
				sleep(1150);
				int calcPos = arm.getCurrentPosition() + adjustArmBy;
				arm.setPower((1000-calcPos)*.00016f);
				/*while(!opModeIsActive())
				{
					int armPos = arm.getCurrentPosition();
					int calcPos = armPos + adjustArmBy;
					arm.setPower((1000-calcPos)*.00013f);
				}*/
				waitForStart();
				while (opModeIsActive()) {
						...
						// telemetry.update(); // If code is broken, try uncommenting this.
						int armPos = arm.getCurrentPosition();
						...
						if((gamepad2.a || gamepad2.b) && !handOpen && !changedHand) { //open
								grabber_servo.setPosition(-10);
								handOpen = true;
								changedHand = true;
						}
						else if((gamepad2.b || gamepad2.a) && handOpen && !changedHand) { //close
								grabber_servo.setPosition(200);
								handOpen = false;
								changedHand = true;
						}
						...
						if(gamepad2.start && !changedArmPos)
						{
							changedArmPos = true;
							adjustArmBy = -armPos;
						}
							...
							changedArmPos = false;
						//telemetry.addData("Hand Open? ",handOpen);
						//telemetry.update();
						...
						// arm controls up / down
						
						// armPos == -1550 at ground
						// armPos == -550 at highest point
						calcPos = armPos + adjustArmBy;
						
						if ((gamepad2.left_bumper || gamepad2.left_stick_y < 0 || gamepad2.dpad_up) && calcPos < 1300)// && (armPos < 1010+adjustArmBy || gamepad2.b))
						{
							//
						...
						else if ((gamepad2.right_bumper || gamepad2.left_stick_y > 0 || gamepad2.dpad_down))// && (armPos > 160+adjustArmBy || gamepad2.b))
						{
							//
							arm.setPower(-.4);
						}
						...
						{
								//arm.setPower(.1);
								//arm.setPower((armPos+adjustArmBy)/5312.5f);
							arm.setPower((1000-calcPos)*.00016f);
						}
						
						telemetry.addData("Arm Position:", armPos+adjustArmBy);
						telemetry.update();
						...
						//Y pos < 0 is back/down
						//Y pos > 0 is forward/up
						//X pos > 0 is right
						//X pos < 0 is left
						...
						if (gamepad1.left_stick_y < deadSpace && yDif > xDif) {
								//BACKWARD
								telemetry.addData("key1", -power);
								backRight.setPower(power);
								frontLeft.setPower(power); //Faulty motor
								frontRight.setPower(-power*.7);
								backLeft.setPower(power);
						}
						else if (gamepad1.left_stick_y > deadSpace && yDif > xDif) {
								//FORWARD
								telemetry.addData("key1", power);
								backRight.setPower(-power);
								frontLeft.setPower(-power); //Faulty motor
						...
						else if (gamepad1.left_stick_x > deadSpace && xDif > yDif) {
								//STRAFE RIGHT
								backRight.setPower(power);
								frontLeft.setPower(-power); //Faulty motor
						...
						{
								//TURN RIGHT
								frontRight.setPower(power*0.5); //Faulty motor
						...
						//telemetry.addData("key2", gamepad1.right_stick_x);
					//telemetry.update();
				}
		}
}
